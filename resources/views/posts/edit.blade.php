     ...
     <main>
        <section class="welcome">
            <div class="contact-form">
                <h2 class="block text-2xl font-medium text-gray-700">ToDo編集</h2>
                <form action="{{ route('posts.update', $post->id) }}" method="POST" class="space-y-4" enctype="multipart/form-data">
                    ...
                    <input type="text" name="title" class="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="{{ $post->title }}">
                        {{-- @error('title')
                        <div class="error">{{ $message }}</div> --}}
                        
                    <br>
                    <span class="block text-lg font-medium text-gray-700">タスク内容</span>
                    <input type="textarea" name="body" class="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="{{ $post->contents }}">
                        {{-- @error('body')
                        <div class="error">{{ $message }}</div> --}}
                    ...
                    <input type="submit" value="編集" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-all duration-300">
